...
def parse_args():
    ...
    parser.add_argument('--image_size', type=int, default=256, help='Size of images for evaluation')
        ...
        for batch_idx, (inputs, targets) in enumerate(tqdm(val_loader, desc="Evaluating")):
            ...
            for i in range(inputs.size(0)):
                ...
                if save_visualizations and vis_count < max_vis_samples:
                    ...
                    plt.title('Input (Glared)')
                    plt.axis('off')
                    
                    plt.subplot(1, 3, 2)
                    plt.imshow(output_img, cmap='gray')
                    plt.title(f'Prediction (PSNR: {psnr:.2f}, SSIM: {ssim:.4f})')
                    plt.axis('off')
